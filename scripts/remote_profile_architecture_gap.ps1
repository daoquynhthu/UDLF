param(
    ...
    [string]$OutputName = "architecture_profile.json"
)
...
& $python scripts\profile_architecture_gap.py `
...
if ($LASTEXITCODE -ne 0) { throw "architecture profiling failed with exit code $LASTEXITCODE" }
