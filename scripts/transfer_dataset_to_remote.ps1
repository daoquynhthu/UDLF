...
if (-not $LocalDataset) { $LocalDataset = Resolve-UdlfConfigValue $workspace "local.fineweb_edu_1b" "UDLF_LOCAL_DATASET" "" -AllowMissing }
...
if (-not $LocalDataset) { throw "Missing local dataset. Set local.fineweb_edu_1b or UDLF_LOCAL_DATASET." }
...
if (-not $ArchivePath) {
    $ArchivePath = Join-Path $env:TEMP "$RemoteDatasetName-udlf-dataset.zip"
}
...
if (-not $SkipArchive) {
    ...
    $env:UDLF_DATASET_SRC = $localDatasetPath
    $env:UDLF_DATASET_ZIP = $ArchivePath
    @'
import os
import sys
import time
...
src = Path(os.environ["UDLF_DATASET_SRC"])
dst = Path(os.environ["UDLF_DATASET_ZIP"])
