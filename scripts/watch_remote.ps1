...
if (-not $RemoteHost) { $RemoteHost = Resolve-UdlfConfigValue $workspace "remote.ssh" "UDLF_REMOTE_SSH" "" -AllowMissing }
if (-not $RemoteRunRoot) { $RemoteRunRoot = Resolve-UdlfConfigValue $workspace "remote.runs" "UDLF_REMOTE_RUNS" "" -AllowMissing }
if (-not $RemoteHost) { throw "Missing remote host. Set remote.ssh or UDLF_REMOTE_SSH." }
if (-not $RemoteRunRoot) { throw "Missing remote runs root. Set remote.runs or UDLF_REMOTE_RUNS." }
